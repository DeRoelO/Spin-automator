...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "gxt_wrapper_results")
...
def run():
    log("=========================================================================")
    log("===   CLICKING GXT WRAPPER DIV #x-menu-el-x-auto-810                  ===")
    log("=========================================================================")
    with sync_playwright() as p:
        ...
        # Click exact wrapper div #x-menu-el-x-auto-810 or div with class x-menu-list-item containing Regulier
        log("5. Klikken op wrapper div van Regulier...")
        wrapper = page.locator(".x-menu-list-item", has_text="Regulier").first
        if wrapper.is_visible():
            wrapper.click(force=True)
            log("   Wrapper div van 'Regulier' succesvol geklikt!")
            page.wait_for_timeout(6000)

            page.screenshot(path=os.path.join(OUTPUT_DIR, "01_after_wrapper_click.png"))

            wins = page.query_selector_all(".x-window")
            log(f"6. Aantal geopende vensters na wrapper click: {len(wins)}")
            ...
            log("   Wrapper div van Regulier niet zichtbaar.")
