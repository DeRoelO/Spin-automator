...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "gxt_event_results")
...
def run():
    log("=========================================================================")
    log("===   GXT NATIVE EVENT DISPATCH TEST: BESTAND -> NIEUWE MAATREGEL      ===")
    log("=========================================================================")
    with sync_playwright() as p:
        ...
        # Dispatch mousedown, mouseup, click sequence via JS
        log("5. Mousedown/mouseup/click dispatchen op 'Regulier'...")
        page.evaluate("""() => {
            const els = document.querySelectorAll('a.x-menu-item, span.x-menu-item-text, .x-menu-list-item');
            for (let el of els) {
        ...
        log(f"6. Geopende vensters na JS event sequence: {len(wins)}")
