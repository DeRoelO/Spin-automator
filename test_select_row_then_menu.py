...
def run():
    log("=== TESTING SELECTING GRID ROW FIRST, THEN BESTAND -> NIEUWE MAATREGEL -> VLUCHTSTROOK/BERM ===")
    with sync_playwright() as p:
        ...
        for idx, w in enumerate(wins):
            if w.is_visible():
                ...
                # Fill inputs
                log("8. Formulier Vluchtstrook/berm invullen...")
                page.evaluate("""() => {
                ...
                for b in all_btns:
                    if b.is_visible() and b.inner_text().strip().lower() == "bewaren":
                        log("9. Klikken op 'Bewaren'...")
                        b.click(force=True)
                        page.wait_for_timeout(8000)
                        break

        # Check if saved
        log("10. Dashboard verversen om opgeslagen concept te verifiëren...")
        page.goto(BASE_URL, wait_until="networkidle")
        ...
        for r_idx, r in enumerate(rows[:8]):
            ...
            if "06.09.2026" in txt:
                log(f"   [🎉🎉🎉] VLUCHTSTROOK/BERM CONCEPT SUCCESVOL VERWERKT EN GEBEWAARD IN HET OVERZICHT: Rij {r_idx}: {txt}")
